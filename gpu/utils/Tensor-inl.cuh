...
Tensor<T, Dim, Contig, IndexT, PtrTraits>::copyFrom(
  ...
  if (t.numElements() > 0) {
    ...
    if (tDev == -1) {
      auto err__ = (cudaMemcpyAsync(this->data_,
                                  t.data(),
                                  this->getSizeInBytes(),
                                  ourDev == -1 ? cudaMemcpyHostToHost :
                                  cudaMemcpyHostToDevice,
                                  stream));
      FAISS_ASSERT_FMT(err__ == cudaSuccess, "CUDA error %d, dst: %p, src: %p, size: %ld", (int) err__, this->data_, t.data(), this->getSizeInBytes());
    } else {
